	...
	Context("Reconcile", func() {
		...
		When("creating valid FAR CR", func() {
			...
			It("should have finalizer, taint, while the two VAs and one pod will be deleted", func() {
				...
				verifyStatusCondition(*underTestFAR, commonConditions.ProcessingType, conditionStatusPointer(metav1.ConditionFalse))
				verifyStatusCondition(*underTestFAR, v1alpha1.FenceAgentActionSucceededType, conditionStatusPointer(metav1.ConditionTrue))
				verifyStatusCondition(*underTestFAR, commonConditions.SucceededType, conditionStatusPointer(metav1.ConditionTrue))
			})
		})
		When("creating invalid FAR CR Name", func() {
			...
			It("should not have a finalizer nor taint, while the two VAs and one pod will remain", func() {
				...
				verifyStatusCondition(*underTestFAR, commonConditions.ProcessingType, nil)
				verifyStatusCondition(*underTestFAR, v1alpha1.FenceAgentActionSucceededType, nil)
				verifyStatusCondition(*underTestFAR, commonConditions.SucceededType, nil)
			})
		})
	})
})
...
func verifyStatusCondition(testFAR v1alpha1.FenceAgentsRemediation, conditionType string, conditionStatus *metav1.ConditionStatus) {
	Eventually(func(g Gomega) {
		g.Expect(k8sClient.Get(context.Background(), client.ObjectKeyFromObject(&testFAR), &testFAR)).To(Succeed())
		condition := meta.FindStatusCondition(testFAR.Status.Conditions, conditionType)
